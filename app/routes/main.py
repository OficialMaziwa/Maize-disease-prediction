...
def is_maize_leaf(image_path):
    try:
        ...
        confidence = min(
            100,
            ((green_ratio * 100 + green_percentage * 100 + (edge_density / 10)) / 3),
        )
        ...
        if is_leaf:
            ...
            return (
                False,
                round(confidence, 1),
                "Not a maize leaf. Please upload an image of a maize leaf.",
            )
    except ImportError:
...
def api_predict():
    ...
    try:
        ...
        try:
            ...
            print(
                f"Leaf validation: is_leaf={is_leaf}, confidence={leaf_confidence}%, message={leaf_message}"
            )
        except Exception as e:
        ...
        if language == "sw":
            display_disease_name = (
                disease_info["disease_name_sw"] if disease_info else disease_name
            )
            description = (
                disease_info["description_sw"]
                if disease_info
                else "Maelezo hayapatikani."
            )
            symptoms = (
                disease_info["symptoms_sw"] if disease_info else "Dalili hazipatikani."
            )
            treatment = (
                disease_info["treatment_sw"]
                if disease_info
                else "Matibabu hayapatikani."
            )
            organic = (
                disease_info.get("organic_treatment_sw", "").split("|")
                if disease_info
                else []
            )
            chemical = (
                disease_info.get("chemical_treatment_sw", "").split("|")
                if disease_info
                else []
            )
            cultural = (
                disease_info.get("cultural_practices_sw", "").split("|")
                if disease_info
                else []
            )
            action = (
                disease_info.get("action_plan_sw", "").split("|")
                if disease_info
                else []
            )
        else:
            display_disease_name = (
                disease_info["disease_name_en"] if disease_info else disease_name
            )
            description = (
                disease_info["description_en"]
                if disease_info
                else "No description available."
            )
            symptoms = (
                disease_info["symptoms_en"]
                if disease_info
                else "No symptoms information available."
            )
            treatment = (
                disease_info["treatment_en"]
                if disease_info
                else "No treatment information available."
            )
            organic = (
                disease_info.get("organic_treatment_en", "").split("|")
                if disease_info
                else []
            )
            chemical = (
                disease_info.get("chemical_treatment_en", "").split("|")
                if disease_info
                else []
            )
            cultural = (
                disease_info.get("cultural_practices_en", "").split("|")
                if disease_info
                else []
            )
            action = (
                disease_info.get("action_plan_en", "").split("|")
                if disease_info
                else []
            )
        ...
        return jsonify(
            {
                ...
                "leaf_message": (
                    "Maize leaf detected"
                    if language == "en"
                    else "Jani la mahindi limegunduliwa"
                ),
                "disease": display_disease_name,
...
def result():
    ...
    description = request.args.get("description") or stored_result.get(
        "description", "No description available."
    )
    symptoms = request.args.get("symptoms") or stored_result.get(
        "symptoms", "No symptoms information available."
    )
    treatment = request.args.get("treatment") or stored_result.get(
        "treatment", "No treatment information available."
    )
    image_data = request.args.get("image_data", "")
    try:
        organic = (
            json.loads(request.args.get("organic"))
            if request.args.get("organic")
            else stored_result.get("organic_treatment", [])
        )
        chemical = (
            json.loads(request.args.get("chemical"))
            if request.args.get("chemical")
            else stored_result.get("chemical_treatment", [])
        )
        cultural = (
            json.loads(request.args.get("cultural"))
            if request.args.get("cultural")
            else stored_result.get("cultural_practices", [])
        )
        action = (
            json.loads(request.args.get("action"))
            if request.args.get("action")
            else stored_result.get("action_plan", [])
        )
    except:
        organic = []
        chemical = []
...
def result():
    ...
    try:
        if ensure_db_connection():
            ...
            if disease_info:
                ...
                if chemical_str:
                    chemical = [item.strip() for item in chemical_str.split("|") if item.strip()]
                if cultural_str:
                    cultural = [item.strip() for item in cultural_str.split("|") if item.strip()]
                if action_str:
                    action = [item.strip() for item in action_str.split("|") if item.strip()]
            else:
                print(f"⚠️ Disease not found: {db_disease_name}")
                # Fallback to stored_result
    ...
    print(f"📊 Organic: {len(organic)}, Chemical: {len(chemical)}, Cultural: {len(cultural)}, Action: {len(action)}")
...
def admin_delete_user(user_id):
    ...
    try:
        cursor = user_db.get_cursor(dictionary=False)
        cursor.execute(
            "DELETE FROM public.diagnosis_history WHERE user_id = %s", (str(user_id),)
        )
        cursor.execute(
            "DELETE FROM public.in_app_notifications WHERE user_id = %s",
            (str(user_id),),
        )
        cursor.execute(
            "DELETE FROM public.notifications WHERE user_id = %s", (str(user_id),)
        )
        cursor.execute(
            "DELETE FROM public.user_activity_logs WHERE user_id = %s", (str(user_id),)
        )
        cursor.execute(
            "DELETE FROM public.user_sessions WHERE user_id = %s", (str(user_id),)
        )
        cursor.execute(
            "DELETE FROM public.login_attempts WHERE user_id = %s", (str(user_id),)
        )
        ...
        if affected_rows > 0:
            log_user_activity(
                "ADMIN_ACTION", f"Deleted user ID: {user_id} with all related data"
            )
            return jsonify(
                {
                    "success": True,
...
def admin_reject_officer(user_id):
    ...
    try:
        ...
        return jsonify(
            {
                "success": True,
                "message": f"Officer {officer['full_name']} rejected successfully.",
            }
        )
    except Exception as e:
...
def admin_add_disease():
    ...
    if request.method == "POST":
        ...
        cursor = user_db.get_cursor()
        cursor.execute(
            "SELECT disease_id FROM diseases WHERE disease_name_en = %s",
            (disease_name_en,),
        )
        if cursor.fetchone():
            cursor.close()
...
def api_officer_dashboard_data():
    """Dashboard data - inafilter kulingana na role ya mtumiaji"""
    ...
    try:
        ...
        print(f"🔍 [DASHBOARD] Role: {user_role}, Region: {officer_region}")

        if user_role == "admin":
            cursor.execute(
                "SELECT COUNT(*) as count FROM maziwa WHERE LOWER(role) = 'farmer'"
            )
        else:
            if officer_region:
                cursor.execute(
                    "SELECT COUNT(*) as count FROM maziwa WHERE LOWER(role) = 'farmer' AND LOWER(region) = LOWER(%s)",
                    (officer_region,),
                )
            else:
                cursor.execute(
        ...
        if user_role == "admin":
            ...
            if officer_region:
                cursor.execute(
                    """
                    SELECT COUNT(DISTINCT dh.user_id) as count 
                    FROM diagnosis_history dh
                    INNER JOIN maziwa m ON dh.user_id::text = m.user_id::text
                    WHERE dh.diagnosis_date::timestamp >= NOW() - INTERVAL '30 days'
                    AND LOWER(m.role) = 'farmer'
                    AND LOWER(m.region) = LOWER(%s)
                """,
                    (officer_region,),
                )
            else:
        ...
        print(f"📊 Active Farmers (30d): {active_farmers}")
        ...
        if user_role == "admin":
            ...
            if officer_region:
                cursor.execute(
                    ...
                    created_at::text as created_at 
                    FROM maziwa 
                    WHERE LOWER(role) = 'farmer' AND LOWER(region) = LOWER(%s)
                    ORDER BY created_at DESC LIMIT 50
                """,
                    (officer_region,),
                )
        ...
        if user_role == "admin":
            cursor.execute("""
                ...
                ORDER BY dh.diagnosis_date DESC LIMIT 50
            """)
        else:
            if officer_region:
                cursor.execute(
                    ...
                    FROM diagnosis_history dh 
                    LEFT JOIN maziwa u ON dh.user_id::text = u.user_id::text 
                    WHERE LOWER(u.region) = LOWER(%s) OR u.region IS NULL
                    ORDER BY dh.diagnosis_date DESC LIMIT 50
                """,
                    (officer_region,),
                )
        ...
        print(
            f"📊 Recent Farmers: {len(recent_farmers)}, Recent Predictions: {len(recent_predictions)}"
        )
...
def api_officer_farmer_diagnoses(user_id):
    ...
    try:
        cursor = user_db.get_cursor()
        cursor.execute(
            "SELECT user_id, full_name, region FROM maziwa WHERE user_id = %s AND role = 'farmer'",
            (user_id,),
        )
            ...
            if officer_region and farmer.get("region") != officer_region:
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "message": "Access denied. Farmer is not in your region.",
                        }
                    ),
                    403,
                )
        ...
        for diagnosis in diagnoses:
            if diagnosis.get("diagnosis_date") and hasattr(
                diagnosis["diagnosis_date"], "strftime"
            ):
                diagnosis["diagnosis_date"] = diagnosis["diagnosis_date"].strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
        return jsonify(
            {
...
def api_officer_get_prediction(prediction_id):
    ...
    try:
        ...
        if prediction.get("diagnosis_date") and hasattr(
            prediction["diagnosis_date"], "strftime"
        ):
            prediction["diagnosis_date"] = prediction["diagnosis_date"].strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        if prediction.get("created_at") and hasattr(
        ...
        if prediction.get("confidence_score"):
            prediction["confidence_score"] = safe_float(prediction["confidence_score"])
...
@main.route("/api/officer/prediction/<prediction_id>/delete", methods=["DELETE"])
@log_activity
def api_officer_delete_prediction(prediction_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, image_path FROM diagnosis_history WHERE id::text = %s",
            (str(prediction_id),),
        )
        prediction = cursor.fetchone()
        ...
        cursor.execute(
            "DELETE FROM diagnosis_history WHERE id::text = %s", (str(prediction_id),)
        )
        db.connection.commit()
        cursor.close()

        log_user_activity(
            "OFFICER_DELETE_PREDICTION",
            f"Officer deleted prediction ID: {prediction_id}",
        )
...
@main.route("/api/officer/prediction/<prediction_id>/download")
@log_activity
...
def download_prediction_csv(prediction):
    ...
    confidence_val = prediction.get("confidence_score", 0)
    confidence_str = f"{safe_float(confidence_val):.2f}%"
...
def api_officer_create_disease():
    ...
    cursor = user_db.get_cursor()
    cursor.execute(
        "SELECT disease_id FROM diseases WHERE disease_name_en = %s", (disease_name_en,)
    )
    if cursor.fetchone():
        cursor.close()
        return (
            jsonify(
                {"success": False, "message": "Disease with this name already exists"}
            ),
            400,
        )
    cursor.execute(
...
def api_officer_delete_disease(disease_id):
    ...
    try:
        cursor = user_db.get_cursor()
        cursor.execute(
            "DELETE FROM public.diagnosis_history WHERE disease_id = %s",
            (str(disease_id),),
        )
        cursor.execute("DELETE FROM diseases WHERE disease_id = %s", (str(disease_id),))
        db.connection.commit()
...
def api_farmer_download_predictions():
    ...
    try:
        ...
        for p in predictions:
            confidence_val = safe_float(p.get("confidence_score", 0))
            writer.writerow(
                [
